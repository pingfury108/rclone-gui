//! 单行文本输入框（基于 GPUI InputHandler，支持中文输入法）
//! 改编自 gpui 官方 examples/input.rs，增加密码掩码模式与主题适配
...
use crate::theme::{color, BORDER, ERROR, TEXT, TEXT_MUTED};
...
const MASK_CHAR: &str = "•";
const MASK_CHAR_LEN: usize = 3; // "•" 的 UTF-8 字节数
...
impl TextInput {
    ...
    pub fn masked(cx: &mut Context<Self>, placeholder: impl Into<SharedString>) -> Self {
    ...
    pub fn set_content(&mut self, value: &str, cx: &mut Context<Self>) {
    ...
    fn display_content(&self) -> SharedString {
        if self.masked && !self.content.is_empty() {
            MASK_CHAR.repeat(self.content.chars().count()).into()
        } else {
            self.content.clone()
        }
    }

    /// content 字节偏移 → display 字节偏移
    ...
    /// display 字节偏移 → content 字节偏移
    fn to_content_offset(&self, display_offset: usize) -> usize {
        if self.masked {
            let chars = display_offset / MASK_CHAR_LEN;
            self.content
                .char_indices()
                .nth(chars)
                .map(|(i, _)| i)
...
impl EntityInputHandler for TextInput {
    ...
    fn character_index_for_point(
        ...
        Some(self.offset_to_utf16(self.to_content_offset(display_index)))
    }
}
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let runs = if let Some(marked_range) = input.marked_range.as_ref() {
            let start = input.to_display_offset(marked_range.start);
            let end = input.to_display_offset(marked_range.end);
            vec![
                TextRun {
                    len: start,
                    ..run.clone()
...
impl Render for TextInput {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let border = if self.invalid { ERROR } else { BORDER };
        div()
            .flex()
...
/// 表单标签
pub fn form_label(text: impl Into<SharedString>) -> gpui::Div {
    div()
        .text_xs()
